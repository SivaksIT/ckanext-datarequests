...
from ckanext import datarequests
import ckanext.datarequests.constants as constants
import ckanext.datarequests.views.datarequests as controller
from ckanext.datarequests.tests.fixtures import datarequest_setup
...
from ckanext.datarequests.tests import factories as request_factory
...
class TestUIController(object):

    # @pytest.mark.usefixture("clean_db")
    ...
    ################################# NEW ################################
    ######################################################################
    @pytest.mark.usefixtures("datarequest_setup")
    def test_basic():
        engine = model.Session.get_bind()
        inspector = inspect(engine)
    
        assert 'datarequests' in inspector.get_table_names()

    # @pytest.mark.usefixtures("clean_db", "datarequest_setup", "with_request_context",)
    # def test_new_no_post(self, app):
    #     user = factories.User()
    #     env = {'REMOTE_USER': user['name'].encode('ascii')}
    #     response = app.get(toolkit.url_for('datarequests.new'),
    #         extra_environ=env,
    #     )
    ...
    # @pytest.mark.usefixtures("clean_db", "datarequest_setup")
    # def test_new_no_post_no_user(self, app):
    #     response = app.get(toolkit.url_for('datarequests.new'))
        
    #     assert response.status_code == 403
    #     assert 'Unauthorized to create a Data Request' in response.body

    # @pytest.mark.usefixtures("clean_db", "with_request_context", "datarequest_setup")
    # def test_new_post_content(self, app):

    #     user = factories.User()
    #     env = {'REMOTE_USER': user['name'].encode('ascii')}

    #     response = app.post(toolkit.url_for('datarequests.new'),
    #         params ={
    #             'title': 'Some Title',
    ...
    #         extra_environ=env,
    #     )    
    #     assert db.DataRequest.datarequest_exists(title='Some Title')
    #     assert response._status_code == 200

    # @pytest.mark.usefixtures("clean_db", "with_request_context", "datarequest_setup")
    # def test_new_post_content_no_user(self, app):
    #     context = {
    #         'user': None,
    #         'model': model,
    #     }
    ...
    #             'description': 'Some Description'
    #         },
    #         extra_environ=context
    #     )    

    #     assert response.status_code == 403
    #     pytest.raises (toolkit.NotAuthorized, helpers.call_auth,
    #     constants.CREATE_DATAREQUEST, context)

    
    # @pytest.mark.usefixtures("clean_db", "datarequest_setup", "with_request_context")
    # def test_factory(slef, app):
    #     engine = model.Session.get_bind()
    #     inspector = inspect(engine)
    #     print('----> TEST_FACTORY', (inspector.get_table_names()))
    #     user = factories.User()
    #     org = factories.Organization()
    #     request_factory.Datarequests(user=user, organization_id=org['id'])

    #     assert db.DataRequest.count() == 1
    ...
    #     print(self.controller_instance)
    #     # request = app.get(
    #     #     url=toolkit.url_for('datarequests.update',
    #     #                         datarequest_id = 'example_uuidv4'))
